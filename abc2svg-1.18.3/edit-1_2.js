...
function do_scroll(elt) {
  var r, dr = elt_ref.target.parentElement,
    ...
    ty = elt_ref.target.getBoundingClientRect().y;
  while(elt.tagName != "svg") elt = elt.parentNode;
  r = elt.getBoundingClientRect();
  if(r.y < 0) dr.scrollTo(0, r.y - ty);
  else if(r.y + r.height > drh) dr.scrollTo(0, r.y - ty - drh + r.height)
}
...
function seltxt(evt) {
  ...
  play.loop = false;
  if(!start) {
    if(end == elt.value.length) return;
    setsel(0, 0);
    setsel(1, 0);
    return
  }
...
function notehlight(i, on) {
  ...
  if(elts && elts[0]) {
    if(on) do_scroll(elts[0]);
    elts[0].style.fillOpacity = on ? .4 : 0
  }
}
...
function play_tune(what) {
  ...
  function go_note(si) {
    for(var i = si;; i++) {
      if(!s[i]) return si;
      if("ABCDEFGabcdefg[^_=".indexOf(s[i]) >= 0) break
    }
    return i
  }
  ctxMenu.style.display = "none";
...
function edit_init() {
  ...
  if(window.AudioContext || window.webkitAudioContext || navigator.requestMIDIAccess) {
    /* **************************************** */
    /* abc2svg.loadjs("play-1.js", function() { */
    abc2svg.loadjs("play-1_2.js", function() {
    /* **************************************** */  
