...
function Audio5(i_conf) {
  ...
  if(!conf.sfu) conf.sfu = "../abc2svg/Scc1t2";
  return {
    get_outputs: function() {
...
function Midi5(i_conf) {
  ...
  function note_run(e, t, d) {
    ...
    if(d && Midi5.ma.sysexEnabled) {
      op.send(new Uint8Array([240, 127, 127, 8, 2, i & 127, 1, k, k, d / .78125, 0, 247]), t)
    }
  ...
  return {
    ...
    play: function(istart, i_iend, a_e) {
      ...
      evt_idx = istart;
      if(0) {
        op.send(new Uint8Array([240, 127, 127, 8, 2, 0, 1, 105, 105, 0, 0, 247]), t)
      }
...
if(navigator.requestMIDIAccess) navigator.requestMIDIAccess({
  sysex: true
}).then(onMIDISuccess, onMIDIFailure1);